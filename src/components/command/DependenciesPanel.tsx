...
import { mockDependencies, DependencyTask } from "@/lib/mock-data";
...
function getUrgency(dueDate: string): { class: string; label: string } {
  ...
  if (hoursLeft < 0) return { class: "text-red-400 bg-red-500/10 border-red-500/30", label: "OVERDUE" };
  if (hoursLeft < 72) return { class: "text-amber-400 bg-amber-500/10 border-amber-500/30", label: "URGENT" };
  return { class: "text-green-400 bg-green-500/10 border-green-500/30", label: "ON TRACK" };
}
...
export default function DependenciesPanel() {
  const [tasks, setTasks] = useState<DependencyTask[]>(mockDependencies);
  const [actioned, setActioned] = useState<Record<string, "approved" | "rejected">>({});

  const handleAction = (taskId: string, action: "approved" | "rejected") => {
    setActioned((prev) => ({ ...prev, [taskId]: action }));
    // In production: POST to /api/tasks/:id/approve or /reject
  };
            ...
            >
              ...
              <div className="flex items-center gap-2 mb-2 flex-wrap">
                ...
                <span className="text-xs text-gray-400">
                  {task.assignedToEmoji} {task.assignedToAgent}
                </span>
              </div>
