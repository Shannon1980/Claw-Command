...
export const isEventDomain = (value: unknown): value is EventDomain => {
  return value === "vorentoe" || value === "skyward" || value === "community" || value === "teaching";
};
...
export function detectConflicts(events: CalendarEvent[]): EventConflict[] {
  ...
  for (let i = 0; i < events.length; i++) {
    for (let j = i + 1; j < events.length; j++) {
      ...
      const overlaps = event1.startTime < event2.endTime && event1.endTime > event2.startTime;
